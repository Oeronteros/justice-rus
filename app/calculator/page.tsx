...
export default function CalculatorPage() {
  ...
  const nameInputClassName = controlDisabled
    ? 'bg-transparent text-xl sm:text-2xl font-bold font-orbitron text-[#8fb9cc] focus:outline-none opacity-60 cursor-not-allowed'
    : 'bg-transparent text-xl sm:text-2xl font-bold font-orbitron text-[#8fb9cc] focus:outline-none';
  ...
  return (
    <section className="py-8 sm:py-10">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="mb-8 grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.75fr)] xl:gap-6">
          ...
          <div className="card flex flex-col justify-between gap-4 p-5 sm:p-6">
            ...
            {controlDisabled && isAuthResolved ? (
              <div className="rounded-2xl border border-[#2a4a5f]/60 bg-[#0c151d]/85 p-4 text-sm text-[#c8dce7]">
                <div className="flex items-start gap-3">
                  <span className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#36566d]/60 bg-[#10202d]/80 text-[#8fb9cc]">
                    <WuxiaIcon name="shield" className="h-4 w-4" />
                  </span>
                  <div className="space-y-1">
                    <div className="text-[11px] uppercase tracking-[0.22em] text-[#8fb9cc]">{copy.previewMode}</div>
                    <div>{copy.accessRestricted}</div>
                  </div>
                </div>
              </div>
            ) : null}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
          {builds.map((build, index) => (
            <div
              ...
              className={`card p-5 sm:p-6 lg:p-8 ${results[index].dps > 0 && results[index].dps === topDps ? 'border-emerald-400/45 shadow-[0_28px_60px_rgba(16,185,129,0.16)]' : ''}`}
            >
