...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/` | `about` | Protected content behind `PortalShell` PinScreen/session | Dashboard section hooks + shell providers | `app/(portal)/page.tsx` | - | Next | Vinext | Blocked | Vinext root route file missing | `VINEXT_CUTOVER_SCOPE=off` |
| `/about` | `about` | Protected content behind `PortalShell` PinScreen/session | Nav alias behavior only | - | - | Next | Vinext | Blocked | No dedicated Next or Vinext `/about` page exists | `VINEXT_CUTOVER_SCOPE=off` |
