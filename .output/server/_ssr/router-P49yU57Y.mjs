...
import { n as appendSearchParamsToUrl, r as urlQueryToSearchParams, t as addQueryParam } from "./query-BDjhEtUw.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/router-P49yU57Y.js
...
function applyNavigationLocale(url, locale) {
	...
	if (locale === window.__VINEXT_DEFAULT_LOCALE__) return url;
...
async function navigateClient(url) {
	...
	try {
		...
		const React = (await import("./react-ZbIu7eLJ.mjs").then((n) => /* @__PURE__ */ __toESM(n.t(), 1))).default;
...
function buildRouterValue(pathname, query, asPath, methods) {
	...
	return {
		pathname,
		route: typeof window !== "undefined" ? window.__NEXT_DATA__?.page ?? pathname : pathname,
		query,
		asPath,
		basePath: __basePath,
		locale,
