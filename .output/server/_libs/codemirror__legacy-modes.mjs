...
var apl_exports = /* @__PURE__ */ __exportAll({ apl: () => apl });
var builtInFuncs = {
	"+": ["conjugate", "add"],
	"−": ["negate", "subtract"],
	"×": ["signOf", "multiply"],
...
var asciiarmor_exports = /* @__PURE__ */ __exportAll({ asciiArmor: () => asciiArmor });
...
var asn1_exports = /* @__PURE__ */ __exportAll({ asn1: () => asn1 });
function words$20(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var defaults$1 = {
	keywords: words$20("DEFINITIONS OBJECTS IF DERIVED INFORMATION ACTION REPLY ANY NAMED CHARACTERIZED BEHAVIOUR REGISTERED WITH AS IDENTIFIED CONSTRAINED BY PRESENT BEGIN IMPORTS FROM UNITS SYNTAX MIN-ACCESS MAX-ACCESS MINACCESS MAXACCESS REVISION STATUS DESCRIPTION SEQUENCE SET COMPONENTS OF CHOICE DistinguishedName ENUMERATED SIZE MODULE END INDEX AUGMENTS EXTENSIBILITY IMPLIED EXPORTS"),
	cmipVerbs: words$20("ACTIONS ADD GET NOTIFICATIONS REPLACE REMOVE"),
	compareTypes: words$20("OPTIONAL DEFAULT MANAGED MODULE-TYPE MODULE_IDENTITY MODULE-COMPLIANCE OBJECT-TYPE OBJECT-IDENTITY OBJECT-COMPLIANCE MODE CONFIRMED CONDITIONAL SUBORDINATE SUPERIOR CLASS TRUE FALSE NULL TEXTUAL-CONVENTION"),
	status: words$20("current deprecated mandatory obsolete"),
	tags: words$20("APPLICATION AUTOMATIC EXPLICIT IMPLICIT PRIVATE TAGS UNIVERSAL"),
	storage: words$20("BOOLEAN INTEGER OBJECT IDENTIFIER BIT OCTET STRING UTCTime InterfaceIndex IANAifType CMIP-Attribute REAL PACKAGE PACKAGES IpAddress PhysAddress NetworkAddress BITS BMPString TimeStamp TimeTicks TruthValue RowStatus DisplayString GeneralString GraphicString IA5String NumericString PrintableString SnmpAdminString TeletexString UTF8String VideotexString VisibleString StringStore ISO646String T61String UniversalString Unsigned32 Integer32 Gauge Gauge32 Counter Counter32 Counter64"),
	modifier: words$20("ATTRIBUTE ATTRIBUTES MANDATORY-GROUP MANDATORY-GROUPS GROUP GROUPS ELEMENTS EQUALITY ORDERING SUBSTRINGS DEFINED"),
	accessTypes: words$20("not-accessible accessible-for-notify read-only read-create read-write"),
...
var asterisk_exports = /* @__PURE__ */ __exportAll({ asterisk: () => asterisk });
var atoms$14 = [
	"exten",
	"same",
	"include",
...
var brainfuck_exports = /* @__PURE__ */ __exportAll({ brainfuck: () => brainfuck });
var reserve = "><+-.,[]".split("");
var brainfuck = {
	name: "brainfuck",
	startState: function() {
...
var clike_exports = /* @__PURE__ */ __exportAll({
	clike: () => clike,
	csharp: () => csharp,
	dart: () => dart,
	kotlin: () => kotlin,
...
function Context$10(indented, column, type, info, align, prev) {
	this.indented = indented;
	this.column = column;
	this.type = type;
	this.info = info;
...
function pushContext$14(state, col, type, info) {
	var indent = state.indented;
	if (state.context && state.context.type == "statement" && type != "statement") indent = state.context.indented;
	return state.context = new Context$10(indent, col, type, info, null, state.context);
}
function popContext$14(state) {
	var t = state.context.type;
	if (t == ")" || t == "]" || t == "}") state.indented = state.context.indented;
	return state.context = state.context.prev;
}
...
function clike(parserConfig) {
	...
	return {
		...
		startState: function(indentUnit) {
			return {
				tokenize: null,
				context: new Context$10(-indentUnit, 0, "top", null, false),
				indented: 0,
				startOfLine: true,
				prevToken: null
			};
		...
		token: function(stream, state) {
			...
			if (ctx.align == null) ctx.align = true;
			if (curPunc == ";" || curPunc == ":" || curPunc == "," && stream.match(/^\s*(?:\/\/.*)?$/, false)) while (state.context.type == "statement") popContext$14(state);
			else if (curPunc == "{") pushContext$14(state, stream.column(), "}");
			else if (curPunc == "[") pushContext$14(state, stream.column(), "]");
			else if (curPunc == "(") pushContext$14(state, stream.column(), ")");
			else if (curPunc == "}") {
				while (ctx.type == "statement") ctx = popContext$14(state);
				if (ctx.type == "}") ctx = popContext$14(state);
				while (ctx.type == "statement") ctx = popContext$14(state);
			} else if (curPunc == ctx.type) popContext$14(state);
			else if (indentStatements && ((ctx.type == "}" || ctx.type == "top") && curPunc != ";" || ctx.type == "statement" && curPunc == "newstatement")) pushContext$14(state, stream.column(), "statement", stream.current());
...
var basicCTypes = words$19("int long char short double float unsigned signed void bool");
var basicObjCTypes = words$19("SEL instancetype id Class Protocol BOOL");
...
clike({
	name: "c",
	keywords: words$19(cKeywords),
	types: cTypes,
	blockKeywords: words$19(cBlockKeywords),
	defKeywords: words$19(cDefKeywords),
	typeFirstDefinitions: true,
	atoms: words$19("NULL true false"),
...
clike({
	name: "cpp",
	keywords: words$19(cKeywords + " " + cppKeywords),
	types: cTypes,
	blockKeywords: words$19(cBlockKeywords + " class try catch"),
	defKeywords: words$19(cDefKeywords + " class namespace"),
	typeFirstDefinitions: true,
	atoms: words$19("true false NULL nullptr"),
...
clike({
	name: "java",
	keywords: words$19("abstract assert break case catch class const continue default do else enum extends final finally for goto if implements import instanceof interface native new package private protected public return static strictfp super switch synchronized this throw throws transient try volatile while @interface"),
	types: words$19("var byte short int long float double boolean char void Boolean Byte Character Double Float Integer Long Number Object Short String StringBuffer StringBuilder Void"),
	blockKeywords: words$19("catch class do else finally for if switch try while"),
	defKeywords: words$19("class interface enum @interface"),
	typeFirstDefinitions: true,
	atoms: words$19("true false null"),
...
var csharp = clike({
	name: "csharp",
	keywords: words$19("abstract as async await base break case catch checked class const continue default delegate do else enum event explicit extern finally fixed for foreach goto if implicit in init interface internal is lock namespace new operator out override params private protected public readonly record ref required return sealed sizeof stackalloc static struct switch this throw try typeof unchecked unsafe using virtual void volatile while add alias ascending descending dynamic from get global group into join let orderby partial remove select set value var yield"),
	types: words$19("Action Boolean Byte Char DateTime DateTimeOffset Decimal Double Func Guid Int16 Int32 Int64 Object SByte Single String Task TimeSpan UInt16 UInt32 UInt64 bool byte char decimal double short int long object sbyte float string ushort uint ulong"),
	blockKeywords: words$19("catch class do else finally for foreach if struct switch try while"),
	defKeywords: words$19("class interface namespace record struct var"),
	typeFirstDefinitions: true,
	atoms: words$19("true false null"),
...
var scala = clike({
	name: "scala",
	keywords: words$19("abstract case catch class def do else extends final finally for forSome if implicit import lazy match new null object override package private protected return sealed super this throw trait try type val var while with yield _ assert assume require print println printf readLine readBoolean readByte readShort readChar readInt readLong readFloat readDouble"),
	types: words$19("AnyVal App Application Array BufferedIterator BigDecimal BigInt Char Console Either Enumeration Equiv Error Exception Fractional Function IndexedSeq Int Integral Iterable Iterator List Map Numeric Nil NotNull Option Ordered Ordering PartialFunction PartialOrdering Product Proxy Range Responder Seq Serializable Set Specializable Stream StringBuilder StringContext Symbol Throwable Traversable TraversableOnce Tuple Unit Vector Boolean Byte Character CharSequence Class ClassLoader Cloneable Comparable Compiler Double Exception Float Integer Long Math Number Object Package Pair Process Runtime Runnable SecurityManager Short StackTraceElement StrictMath String StringBuffer System Thread ThreadGroup ThreadLocal Throwable Triple Void"),
	multiLineStrings: true,
	blockKeywords: words$19("catch class enum do else finally for forSome if match switch try while"),
	defKeywords: words$19("class enum def object package trait type val var"),
	atoms: words$19("true false null"),
	...
	hooks: {
		...
		"=": function(stream, state) {
			...
			if (cx.type == "}" && cx.align && stream.eat(">")) {
				state.context = new Context$10(cx.indented, cx.column, cx.type, cx.info, null, cx.prev);
				return "operator";
			} else return false;
		},
		"/": function(stream, state) {
...
var kotlin = clike({
	name: "kotlin",
	keywords: words$19("package as typealias class interface this super val operator var fun for is in This throw return annotation break continue object if else while do try when !in !is as? file import where by get set abstract enum open inner override private public internal protected catch finally out final vararg reified dynamic companion constructor init sealed field property receiver param sparam lateinit data inline noinline tailrec external annotation crossinline const operator infix suspend actual expect setparam"),
	types: words$19("Boolean Byte Character CharSequence Class ClassLoader Cloneable Comparable Compiler Double Exception Float Integer Long Math Number Object Package Pair Process Runtime Runnable SecurityManager Short StackTraceElement StrictMath String StringBuffer System Thread ThreadGroup ThreadLocal Throwable Triple Void Annotation Any BooleanArray ByteArray Char CharArray DeprecationLevel DoubleArray Enum FloatArray Function Int IntArray Lazy LazyThreadSafetyMode LongArray Nothing ShortArray Unit"),
	intendSwitch: false,
	indentStatements: false,
	multiLineStrings: true,
	number: /^(?:0x[a-f\d_]+|0b[01_]+|(?:[\d_]+(\.\d+)?|\.\d+)(?:e[-+]?[\d_]+)?)(ul?|l|f)?/i,
	blockKeywords: words$19("catch class do else finally for if where try while enum"),
	defKeywords: words$19("class val var object interface fun"),
	atoms: words$19("true false null this"),
	hooks: {
		"@": function(stream) {
			stream.eatWhile(/[\w\$_]/);
			return "meta";
...
clike({
	name: "shader",
	keywords: words$19("sampler1D sampler2D sampler3D samplerCube sampler1DShadow sampler2DShadow const attribute uniform varying break continue discard return for while do if else struct in out inout"),
	types: words$19("float int bool void vec2 vec3 vec4 ivec2 ivec3 ivec4 bvec2 bvec3 bvec4 mat2 mat3 mat4"),
	blockKeywords: words$19("for while do if else struct"),
	builtin: words$19("radians degrees sin cos tan asin acos atan pow exp log exp2 sqrt inversesqrt abs sign floor ceil fract mod min max clamp mix step smoothstep length distance dot cross normalize ftransform faceforward reflect refract matrixCompMult lessThan lessThanEqual greaterThan greaterThanEqual equal notEqual any all not texture1D texture1DProj texture1DLod texture1DProjLod texture2D texture2DProj texture2DLod texture2DProjLod texture3D texture3DProj texture3DLod texture3DProjLod textureCube textureCubeLod shadow1D shadow2D shadow1DProj shadow2DProj shadow1DLod shadow2DLod shadow1DProjLod shadow2DProjLod dFdx dFdy fwidth noise1 noise2 noise3 noise4"),
	atoms: words$19("true false gl_FragColor gl_SecondaryColor gl_Normal gl_Vertex gl_MultiTexCoord0 gl_MultiTexCoord1 gl_MultiTexCoord2 gl_MultiTexCoord3 gl_MultiTexCoord4 gl_MultiTexCoord5 gl_MultiTexCoord6 gl_MultiTexCoord7 gl_FogCoord gl_PointCoord gl_Position gl_PointSize gl_ClipVertex gl_FrontColor gl_BackColor gl_FrontSecondaryColor gl_BackSecondaryColor gl_TexCoord gl_FogFragCoord gl_FragCoord gl_FrontFacing gl_FragData gl_FragDepth gl_ModelViewMatrix gl_ProjectionMatrix gl_ModelViewProjectionMatrix gl_TextureMatrix gl_NormalMatrix gl_ModelViewMatrixInverse gl_ProjectionMatrixInverse gl_ModelViewProjectionMatrixInverse gl_TextureMatrixTranspose gl_ModelViewMatrixInverseTranspose gl_ProjectionMatrixInverseTranspose gl_ModelViewProjectionMatrixInverseTranspose gl_TextureMatrixInverseTranspose gl_NormalScale gl_DepthRange gl_ClipPlane gl_Point gl_FrontMaterial gl_BackMaterial gl_LightSource gl_LightModel gl_FrontLightModelProduct gl_BackLightModelProduct gl_TextureColor gl_EyePlaneS gl_EyePlaneT gl_EyePlaneR gl_EyePlaneQ gl_FogParameters gl_MaxLights gl_MaxClipPlanes gl_MaxTextureUnits gl_MaxTextureCoords gl_MaxVertexAttribs gl_MaxVertexUniformComponents gl_MaxVaryingFloats gl_MaxVertexTextureImageUnits gl_MaxTextureImageUnits gl_MaxFragmentUniformComponents gl_MaxCombineTextureImageUnits gl_MaxDrawBuffers"),
	indentSwitch: false,
	hooks: { "#": cppHook }
});
clike({
	name: "nesc",
	keywords: words$19(cKeywords + " as atomic async call command component components configuration event generic implementation includes interface module new norace nx_struct nx_union post provides signal task uses abstract extends"),
	types: cTypes,
	blockKeywords: words$19(cBlockKeywords),
	atoms: words$19("null true false"),
	hooks: { "#": cppHook }
...
var objectiveC = clike({
	name: "objectivec",
	keywords: words$19(cKeywords + " " + objCKeywords),
	types: objCTypes,
	builtin: words$19(objCBuiltins),
	blockKeywords: words$19(cBlockKeywords + " @synthesize @try @catch @finally @autoreleasepool @synchronized"),
	defKeywords: words$19(cDefKeywords + " @interface @implementation @protocol @class"),
	dontIndentStatements: /^@.*$/,
	typeFirstDefinitions: true,
	atoms: words$19("YES NO NULL Nil nil true false nullptr"),
	isReservedIdentifier: cIsReservedIdentifier,
	hooks: {
		"#": cppHook,
		"*": pointerHook
...
var objectiveCpp = clike({
	name: "objectivecpp",
	keywords: words$19(cKeywords + " " + objCKeywords + " " + cppKeywords),
	types: objCTypes,
	builtin: words$19(objCBuiltins),
	blockKeywords: words$19(cBlockKeywords + " @synthesize @try @catch @finally @autoreleasepool @synchronized class try catch"),
	defKeywords: words$19(cDefKeywords + " @interface @implementation @protocol @class class namespace"),
	dontIndentStatements: /^@.*$|^template$/,
	typeFirstDefinitions: true,
	atoms: words$19("YES NO NULL Nil nil true false nullptr"),
	isReservedIdentifier: cIsReservedIdentifier,
...
var squirrel = clike({
	name: "squirrel",
	keywords: words$19("base break clone continue const default delete enum extends function in class foreach local resume return this throw typeof yield constructor instanceof static"),
	types: cTypes,
	blockKeywords: words$19("case catch class else for foreach if switch try while"),
	defKeywords: words$19("function local class"),
	typeFirstDefinitions: true,
	atoms: words$19("true false null"),
	hooks: { "#": cppHook }
});
...
clike({
	name: "ceylon",
	keywords: words$19("abstracts alias assembly assert assign break case catch class continue dynamic else exists extends finally for function given if import in interface is let module new nonempty object of out outer package return satisfies super switch then this throw try value void while"),
	types: function(word) {
		var first = word.charAt(0);
		return first === first.toUpperCase() && first !== first.toLowerCase();
	},
	blockKeywords: words$19("case catch class dynamic else finally for function if interface module new object switch try while"),
	defKeywords: words$19("class dynamic function interface module object package value"),
	builtin: words$19("abstract actual aliased annotation by default deprecated doc final formal late license native optional sealed see serializable shared suppressWarnings tagged throws variable"),
	isPunctuationChar: /[\[\]{}\(\),;\:\.`]/,
	isOperatorChar: /[+\-*&%=<>!?|^~:\/]/,
	numberStart: /[\d#$]/,
	...
	multiLineStrings: true,
	typeFirstDefinitions: true,
	atoms: words$19("true false null larger smaller equal empty finished"),
	indentSwitch: false,
	styleDefs: false,
	hooks: {
		"@": function(stream) {
...
var clojure_exports = /* @__PURE__ */ __exportAll({ clojure: () => clojure });
var atoms$13 = [
	"false",
	"nil",
	"true"
...
var atom$1 = createLookupMap(atoms$13);
...
var clojure = {
	...
	languageData: {
		...
		commentTokens: { line: ";;" },
		autocomplete: [].concat(atoms$13, specialForms, coreSymbols)
	}
};
//#endregion
//#region node_modules/@codemirror/legacy-modes/mode/cmake.js
var cmake_exports = /* @__PURE__ */ __exportAll({ cmake: () => cmake });
var variable_regex$1 = /({)?[a-zA-Z0-9_]+(})?/;
function tokenString$26(stream, state) {
...
function tokenize$4(stream, state) {
	...
	if (state.continueString) {
		stream.backUp(1);
		return tokenString$26(stream, state);
	}
...
var cobol_exports = /* @__PURE__ */ __exportAll({ cobol: () => cobol });
	...
	token: function(stream, state) {
		...
		switch (state.mode) {
			...
			default:
				...
				else if (isNumber$1(ch, stream)) returnType = NUMBER$1;
				else {
					if (stream.current().match(tests.symbol)) while (col < 71) if (stream.eat(tests.symbol) === void 0) break;
					else col++;
					if (keywords$37 && keywords$37.propertyIsEnumerable(stream.current().toUpperCase())) returnType = KEYWORD;
...
var coffeescript_exports = /* @__PURE__ */ __exportAll({ coffeeScript: () => coffeeScript });
...
var keywords$36 = wordRegexp$14(indentKeywords$1.concat([
	"break",
	"by",
	"continue",
	"debugger",
...
function tokenBase$43(stream, state) {
	if (stream.sol()) {
	...
	if (stream.match(keywords$36)) return "keyword";
	...
	return function(stream, state) {
		while (!stream.eol()) {
			...
			if (stream.eat("\\")) {
				...
				state.tokenize = tokenBase$43;
				return outclass;
			} else stream.eat(/['"\/]/);
		}
		if (singleline) state.tokenize = tokenBase$43;
		return outclass;
	};
}
...
var coffeeScript = {
	...
	indent: function(state, text) {
		if (state.tokenize != tokenBase$43) return 0;
...
var commonlisp_exports = /* @__PURE__ */ __exportAll({ commonLisp: () => commonLisp });
...
var crystal_exports = /* @__PURE__ */ __exportAll({ crystal: () => crystal });
function wordRegExp(words, end) {
	return new RegExp((end ? "" : "^") + "(?:" + words.join("|") + ")" + (end ? "$" : "\\b"));
}
function chain$10(tokenize, stream, state) {
...
function tokenBase$42(stream, state) {
	if (stream.eatSpace()) return null;
	if (state.lastToken != "\\" && stream.match("{%", false)) return chain$10(tokenMacro("%", "%"), stream, state);
	if (state.lastToken != "\\" && stream.match("{{", false)) return chain$10(tokenMacro("{", "}"), stream, state);
	if (stream.peek() == "#") {
		stream.skipToEnd();
		return "comment";
	...
	if (stream.match(idents)) {
		...
		else if (keywords$35.test(matched)) {
	...
	if (stream.eat("@")) {
		if (stream.peek() == "[") return chain$10(tokenNest("[", "]", "meta"), stream, state);
		stream.eat("@");
	...
	if (stream.eat(":")) {
		if (stream.eat("\"")) return chain$10(tokenQuote("\"", "atom", false), stream, state);
	...
	if (stream.eat("\"")) return chain$10(tokenQuote("\"", "string", true), stream, state);
	if (stream.peek() == "%") {
		var style = "string";
		var embed = true;
		var delim;
		...
		return chain$10(tokenQuote(delim, style, embed), stream, state);
	}
	if (matched = stream.match(/^<<-('?)([A-Z]\w*)\1/)) return chain$10(tokenHereDoc(matched[2], !matched[1]), stream, state);
	if (stream.eat("'")) {
	...
	if (matched = stream.match(/[({[]/, false)) {
		matched = matched[0];
		return chain$10(tokenNest(matched, matching[matched], null), stream, state);
	}
	if (stream.eat("\\")) {
		stream.next();
		return "meta";
...
function tokenNest(begin, end, style, started) {
	return function(stream, state) {
		...
		var nextStyle = tokenBase$42(stream, state);
...
function tokenMacro(begin, end, started) {
	return function(stream, state) {
		...
		return tokenBase$42(stream, state);
	};
}
...
var css_exports = /* @__PURE__ */ __exportAll({
	gss: () => gss,
	mkCSS: () => mkCSS
});
...
var cypher_exports = /* @__PURE__ */ __exportAll({ cypher: () => cypher });
var wordRegexp$13 = function(words) {
	return new RegExp("^(?:" + words.join("|") + ")$", "i");
};
var tokenBase$41 = function(stream) {
	curPunc$11 = null;
	var ch = stream.next();
	if (ch === "\"") {
		stream.match(/^.*?"/);
	...
	if (/[{}\(\),\.;\[\]]/.test(ch)) {
		...
		if (keywords$34.test(word) || systemKeywords.test(word)) return "keyword";
		return "variable";
	}
};
var pushContext$13 = function(state, type, col) {
...
var popContext$13 = function(state) {
...
var cypher = {
	...
	token: function(stream, state) {
		...
		if (curPunc$11 === "(") pushContext$13(state, ")", stream.column());
		else if (curPunc$11 === "[") pushContext$13(state, "]", stream.column());
		else if (curPunc$11 === "{") pushContext$13(state, "}", stream.column());
		else if (/[\]\}\)]/.test(curPunc$11)) {
			while (state.context && state.context.type === "pattern") popContext$13(state);
			if (state.context && curPunc$11 === state.context.type) popContext$13(state);
		} else if (curPunc$11 === "." && state.context && state.context.type === "pattern") popContext$13(state);
		else if (/atom|string|variable/.test(style) && state.context) {
			if (/[\}\]]/.test(state.context.type)) pushContext$13(state, "pattern", stream.column());
...
var d_exports = /* @__PURE__ */ __exportAll({ d: () => d$1 });
...
var parserConfig$2 = {
	keywords: words$18("abstract alias align asm assert auto break case cast cdouble cent cfloat const continue debug default delegate delete deprecated export extern final finally function goto immutable import inout invariant is lazy macro module new nothrow override package pragma private protected public pure ref return shared short static super synchronized template this throw typedef typeid typeof volatile __FILE__ __LINE__ __gshared __traits __vector __parameters " + blockKeywordsStr),
	blockKeywords: words$18(blockKeywordsStr),
	builtin: words$18("bool byte char creal dchar double float idouble ifloat int ireal long real short ubyte ucent uint ulong ushort wchar wstring void size_t sizediff_t"),
	atoms: words$18("exit failure success true false null"),
	hooks: { "@": function(stream, _state) {
		stream.eatWhile(/[\w\$_]/);
		return "meta";
	} }
};
var statementIndentUnit = parserConfig$2.statementIndentUnit, keywords$33 = parserConfig$2.keywords, builtin$3 = parserConfig$2.builtin, blockKeywords$4 = parserConfig$2.blockKeywords, atoms$11 = parserConfig$2.atoms, hooks$2 = parserConfig$2.hooks, multiLineStrings$3 = parserConfig$2.multiLineStrings;
var isOperatorChar$13 = /[+\-*&%=<>!?|\/]/;
var curPunc$10;
function tokenBase$40(stream, state) {
	var ch = stream.next();
	if (hooks$2[ch]) {
	...
	if (ch == "\"" || ch == "'" || ch == "`") {
		state.tokenize = tokenString$25(ch);
		return state.tokenize(stream, state);
	}
	if (/[\[\]{}\(\),;\:\.]/.test(ch)) {
		curPunc$10 = ch;
	...
	if (ch == "/") {
		...
		if (stream.eat("*")) {
			state.tokenize = tokenComment$18;
			return tokenComment$18(stream, state);
		}
		if (stream.eat("/")) {
			stream.skipToEnd();
			return "comment";
	...
	if (isOperatorChar$13.test(ch)) {
		stream.eatWhile(isOperatorChar$13);
		return "operator";
	}
	stream.eatWhile(/[\w\$_\xa1-\uffff]/);
	var cur = stream.current();
	if (keywords$33.propertyIsEnumerable(cur)) {
	...
	if (atoms$11.propertyIsEnumerable(cur)) return "atom";
	return "variable";
}
function tokenString$25(quote) {
	return function(stream, state) {
		var escaped = false, next, end = false;
		while ((next = stream.next()) != null) {
			if (next == quote && !escaped) {
...
function tokenComment$18(stream, state) {
	var maybeEnd = false, ch;
	while (ch = stream.next()) {
		if (ch == "/" && maybeEnd) {
			state.tokenize = null;
...
function pushContext$12(state, col, type) {
	var indent = state.indented;
	if (state.context && state.context.type == "statement") indent = state.context.indented;
	return state.context = new Context$9(indent, col, type, null, state.context);
}
...
var d$1 = {
	...
	token: function(stream, state) {
		...
		if (stream.eatSpace()) return null;
		curPunc$10 = null;
		var style = (state.tokenize || tokenBase$40)(stream, state);
		if (style == "comment" || style == "meta") return style;
		if (ctx.align == null) ctx.align = true;
		if ((curPunc$10 == ";" || curPunc$10 == ":" || curPunc$10 == ",") && ctx.type == "statement") popContext$12(state);
		else if (curPunc$10 == "{") pushContext$12(state, stream.column(), "}");
		else if (curPunc$10 == "[") pushContext$12(state, stream.column(), "]");
		else if (curPunc$10 == "(") pushContext$12(state, stream.column(), ")");
		else if (curPunc$10 == "}") {
			...
			while (ctx.type == "statement") ctx = popContext$12(state);
		} else if (curPunc$10 == ctx.type) popContext$12(state);
		else if ((ctx.type == "}" || ctx.type == "top") && curPunc$10 != ";" || ctx.type == "statement" && curPunc$10 == "newstatement") pushContext$12(state, stream.column(), "statement");
		state.startOfLine = false;
		return style;
	},
	indent: function(state, textAfter, cx) {
		if (state.tokenize != tokenBase$40 && state.tokenize != null) return null;
		var ctx = state.context, firstChar = textAfter && textAfter.charAt(0);
		if (ctx.type == "statement" && firstChar == "}") ctx = ctx.prev;
		var closing = firstChar == ctx.type;
...
var diff_exports = /* @__PURE__ */ __exportAll({ diff: () => diff });
var TOKEN_NAMES = {
	"+": "inserted",
	"-": "deleted",
	"@": "meta"
...
var dockerfile_exports = /* @__PURE__ */ __exportAll({ dockerFile: () => dockerFile });
var from = "from";
...
var dtd_exports = /* @__PURE__ */ __exportAll({ dtd: () => dtd });
var type$4;
function ret$2(style, tp) {
	type$4 = tp;
	return style;
}
function tokenBase$39(stream, state) {
	...
	else if (ch.match(/[\(\)\[\]\-\.,\+\?>]/)) return ret$2(null, ch);
	else if (ch.match(/[\[\]]/)) return ret$2("rule", ch);
	else if (ch == "\"" || ch == "'") {
		state.tokenize = tokenString$24(ch);
		return state.tokenize(stream, state);
...
var dtd = {
	...
	indent: function(state, textAfter, cx) {
		...
		if (textAfter.charAt(0) === "]") n--;
			...
			if (textAfter.substr(0, 1) === "<") {} else if (type$4 == "doindent" && textAfter.length > 1) {} else if (type$4 == "doindent") n--;
			else if (type$4 == ">" && textAfter.length > 1) {} else if (type$4 == "tag" && textAfter !== ">") {} else if (type$4 == "tag" && state.stack[state.stack.length - 1] == "rule") n--;
...
var dylan_exports = /* @__PURE__ */ __exportAll({ dylan: () => dylan });
...
words$17["otherDefinition"] = words$17["unnamedDefinition"].concat(words$17["namedDefinition"]).concat(words$17["otherParameterizedDefinition"]);
words$17["definition"] = words$17["typeParameterizedDefinition"].concat(words$17["otherDefinition"]);
words$17["parameterizedDefinition"] = words$17["typeParameterizedDefinition"].concat(words$17["otherParameterizedDefinition"]);
words$17["simpleDefinition"] = words$17["constantSimpleDefinition"].concat(words$17["variableSimpleDefinition"]).concat(words$17["otherSimpleDefinition"]);
words$17["keyword"] = words$17["statement"].concat(words$17["separator"]).concat(words$17["other"]);
...
forEach([
	...
	forEach(words$17[type], function(word) {
...
function tokenBase$38(stream, state) {
	var ch = stream.peek();
	if (ch == "'" || ch == "\"") {
		stream.next();
		return chain$9(stream, state, tokenString$23(ch, "string"));
	} else if (ch == "/") {
		stream.next();
		if (stream.eat("*")) return chain$9(stream, state, tokenComment$17);
		else if (stream.eat("/")) {
			stream.skipToEnd();
			return "comment";
		}
		...
		if (ch == "\"") {
			stream.next();
			return chain$9(stream, state, tokenString$23("\"", "string"));
		} else if (ch == "b") {
			stream.next();
			stream.eatWhile(/[01]/);
			return "number";
...
function tokenString$23(quote, style) {
	return function(stream, state) {
		var escaped = false, next, end = false;
		while ((next = stream.next()) != null) {
			if (next == quote && !escaped) {
		...
		if (end || !escaped) state.tokenize = tokenBase$38;
		return style;
	};
}
var dylan = {
...
var ebnf_exports = /* @__PURE__ */ __exportAll({ ebnf: () => ebnf });
var commentType = {
	slash: 0,
	parenthesis: 1
};
...
var ecl_exports = /* @__PURE__ */ __exportAll({ ecl: () => ecl });
...
var keyword = words$16("abs acos allnodes ascii asin asstring atan atan2 ave case choose choosen choosesets clustersize combine correlation cos cosh count covariance cron dataset dedup define denormalize distribute distributed distribution ebcdic enth error evaluate event eventextra eventname exists exp failcode failmessage fetch fromunicode getisvalid global graph group hash hash32 hash64 hashcrc hashmd5 having if index intformat isvalid iterate join keyunicode length library limit ln local log loop map matched matchlength matchposition matchtext matchunicode max merge mergejoin min nolocal nonempty normalize parse pipe power preload process project pull random range rank ranked realformat recordof regexfind regexreplace regroup rejected rollup round roundup row rowdiff sample set sin sinh sizeof soapcall sort sorted sqrt stepped stored sum table tan tanh thisnode topn tounicode transfer trim truncate typeof ungroup unicodeorder variance which workunit xmldecode xmlencode xmltext xmlunicode");
var variable = words$16("apply assert build buildindex evaluate fail keydiff keypatch loadxml nothor notify output parallel sequential soapcall wait");
var variable_2 = words$16("__compressed__ all and any as atmost before beginc++ best between case const counter csv descend encrypt end endc++ endmacro except exclusive expire export extend false few first flat from full function group header heading hole ifblock import in interface joined keep keyed last left limit load local locale lookup macro many maxcount maxlength min skew module named nocase noroot noscan nosort not of only opt or outer overwrite packed partition penalty physicallength pipe quote record relationship repeat return right scan self separator service shared skew skip sql store terminator thor threshold token transform trim true type unicodeorder unsorted validate virtual whole wild within xml xpath");
var variable_3 = words$16("ascii big_endian boolean data decimal ebcdic integer pattern qstring real record rule set of string token udecimal unicode unsigned varstring varunicode");
var builtin$2 = words$16("checkpoint deprecated failcode failmessage failure global independent onwarning persist priority recovery stored success wait when");
var blockKeywords$3 = words$16("catch class do else finally for if switch try while");
var atoms$10 = words$16("true false null");
var hooks$1 = { "#": metaHook$1 };
var isOperatorChar$12 = /[+\-*&%=<>!?|\/]/;
var curPunc$9;
function tokenBase$37(stream, state) {
	...
	if (ch == "/") {
		if (stream.eat("*")) {
			state.tokenize = tokenComment$16;
			return tokenComment$16(stream, state);
		}
		if (stream.eat("/")) {
			stream.skipToEnd();
			return "comment";
	...
	if (isOperatorChar$12.test(ch)) {
		stream.eatWhile(isOperatorChar$12);
		return "operator";
	}
	stream.eatWhile(/[\w\$_]/);
...
function tokenString$22(quote) {
	return function(stream, state) {
		...
		if (end || !escaped) state.tokenize = tokenBase$37;
		return "string";
	};
}
...
var ecl = {
	...
	token: function(stream, state) {
		...
		if (stream.eatSpace()) return null;
		curPunc$9 = null;
		var style = (state.tokenize || tokenBase$37)(stream, state);
		if (style == "comment" || style == "meta") return style;
		if (ctx.align == null) ctx.align = true;
		if ((curPunc$9 == ";" || curPunc$9 == ":") && ctx.type == "statement") popContext$11(state);
		else if (curPunc$9 == "{") pushContext$11(state, stream.column(), "}");
		else if (curPunc$9 == "[") pushContext$11(state, stream.column(), "]");
		else if (curPunc$9 == "(") pushContext$11(state, stream.column(), ")");
		else if (curPunc$9 == "}") {
			while (ctx.type == "statement") ctx = popContext$11(state);
			if (ctx.type == "}") ctx = popContext$11(state);
			while (ctx.type == "statement") ctx = popContext$11(state);
		} else if (curPunc$9 == ctx.type) popContext$11(state);
		else if (ctx.type == "}" || ctx.type == "top" || ctx.type == "statement" && curPunc$9 == "newstatement") pushContext$11(state, stream.column(), "statement");
		state.startOfLine = false;
		return style;
	},
	indent: function(state, textAfter, cx) {
		if (state.tokenize != tokenBase$37 && state.tokenize != null) return 0;
		var ctx = state.context, firstChar = textAfter && textAfter.charAt(0);
		if (ctx.type == "statement" && firstChar == "}") ctx = ctx.prev;
		var closing = firstChar == ctx.type;
...
var eiffel_exports = /* @__PURE__ */ __exportAll({ eiffel: () => eiffel });
function wordObj$2(words) {
	var o = {};
	for (var i = 0, e = words.length; i < e; ++i) o[words[i]] = true;
	return o;
}
var keywords$32 = wordObj$2([
	"note",
	"across",
	"when",
	"variant",
...
function tokenBase$36(stream, state) {
	if (stream.eatSpace()) return null;
	var ch = stream.next();
	if (ch == "\"" || ch == "'") return chain$8(readQuoted$1(ch, "string"), stream, state);
	else if (ch == "-" && stream.eat("-")) {
		stream.skipToEnd();
		return "comment";
...
var elm_exports = /* @__PURE__ */ __exportAll({ elm: () => elm });
...
var erlang_exports = /* @__PURE__ */ __exportAll({ erlang: () => erlang });
var typeWords = [
	"-type",
	"-spec",
	"-export_type",
...
var factor_exports = /* @__PURE__ */ __exportAll({ factor: () => factor });
...
var fcl_exports = /* @__PURE__ */ __exportAll({ fcl: () => fcl });
var keywords$31 = {
	"term": true,
	"method": true,
	"accu": true,
...
function tokenBase$35(stream, state) {
	...
	stream.eatWhile(/[\w\$_\xa1-\uffff]/);
	var cur = stream.current().toLowerCase();
	if (keywords$31.propertyIsEnumerable(cur) || start_blocks.propertyIsEnumerable(cur) || end_blocks.propertyIsEnumerable(cur)) return "keyword";
...
var fcl = {
	...
	token: function(stream, state) {
		...
		if (stream.eatSpace()) return null;
		var style = (state.tokenize || tokenBase$35)(stream, state);
		if (style == "comment") return style;
		if (ctx.align == null) ctx.align = true;
		var cur = stream.current().toLowerCase();
		if (start_blocks.propertyIsEnumerable(cur)) pushContext$10(state, stream.column(), "end_block");
		else if (end_blocks.propertyIsEnumerable(cur)) popContext$10(state);
		state.startOfLine = false;
		return style;
	},
	indent: function(state, textAfter, cx) {
		if (state.tokenize != tokenBase$35 && state.tokenize != null) return 0;
		var ctx = state.context;
...
var forth_exports = /* @__PURE__ */ __exportAll({ forth: () => forth });
...
var fortran_exports = /* @__PURE__ */ __exportAll({ fortran: () => fortran });
function words$15(array) {
	var keys = {};
	for (var i = 0; i < array.length; ++i) keys[array[i]] = true;
	return keys;
}
var keywords$30 = words$15([
	"abstract",
	"accept",
	"allocatable",
	"allocate",
...
function tokenBase$34(stream, state) {
	...
	if (ch == "\"" || ch == "'") {
		state.tokenize = tokenString$21(ch);
		return state.tokenize(stream, state);
	}
	...
	stream.eatWhile(/[\w\$_]/);
	var word = stream.current().toLowerCase();
	if (keywords$30.hasOwnProperty(word)) return "keyword";
	if (builtins$7.hasOwnProperty(word) || dataTypes.hasOwnProperty(word)) return "builtin";
	return "variable";
}
function tokenString$21(quote) {
	return function(stream, state) {
		var escaped = false, next, end = false;
		while ((next = stream.next()) != null) {
			if (next == quote && !escaped) {
...
var fortran = {
	...
	token: function(stream, state) {
		if (stream.eatSpace()) return null;
		var style = (state.tokenize || tokenBase$34)(stream, state);
		if (style == "comment" || style == "meta") return style;
		return style;
	}
};
...
var gas_exports = /* @__PURE__ */ __exportAll({ gas: () => gas });
function mkGas(arch) {
	var custom = [];
...
var gherkin_exports = /* @__PURE__ */ __exportAll({ gherkin: () => gherkin });
var gherkin = {
	name: "gherkin",
	startState: function() {
		return {
...
var groovy_exports = /* @__PURE__ */ __exportAll({ groovy: () => groovy });
function words$14(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var keywords$29 = words$14("abstract as assert boolean break byte case catch char class const continue def default do double else enum extends final finally float for goto if implements import in instanceof int interface long native new package private protected public return short static strictfp super switch synchronized threadsafe throw throws trait transient try void volatile while");
var blockKeywords$2 = words$14("catch class def do else enum finally for if interface switch trait try while");
var standaloneKeywords = words$14("return break continue");
var atoms$8 = words$14("null true false this");
var curPunc$8;
function tokenBase$33(stream, state) {
	...
	if (keywords$29.propertyIsEnumerable(cur)) {
...
function tokenBaseUntilBrace$1() {
	...
	function t(stream, state) {
		...
		return tokenBase$33(stream, state);
	}
	t.isBase = true;
	return t;
}
...
var groovy = {
	...
	token: function(stream, state) {
		...
		else if (ctx.type == "}" || ctx.type == "top" || ctx.type == "statement" && curPunc$8 == "newstatement") pushContext$9(state, stream.column(), "statement");
		state.startOfLine = false;
...
var haskell_exports = /* @__PURE__ */ __exportAll({ haskell: () => haskell });
...
var haxe_exports = /* @__PURE__ */ __exportAll({
	haxe: () => haxe,
	hxml: () => hxml
});
function kw(type) {
...
function haxeTokenBase(stream, state) {
	var ch = stream.next();
	if (ch == "\"" || ch == "'") return chain$7(stream, state, haxeTokenString(ch));
	else if (/[\[\]{}\(\),;\:\.]/.test(ch)) return ret$1(ch);
		...
		if (/[A-Z]/.test(ch)) {
			...
			stream.eatWhile(/[\w_]/);
			var word = stream.current(), known = keywords$28.propertyIsEnumerable(word) && keywords$28[word];
...
function statement(type) {
	...
	if (type == "{") return cont$1(pushlex("}"), pushcontext, block$1, poplex, popcontext);
	if (type == ";") return cont$1();
	if (type == "attribute") return cont$1(maybeattribute);
	if (type == "function") return cont$1(functiondef);
	if (type == "for") return cont$1(pushlex("form"), expect("("), pushlex(")"), forspec1, expect(")"), poplex, statement, poplex);
	if (type == "variable") return cont$1(pushlex("stat"), maybelabel);
	if (type == "switch") return cont$1(pushlex("form"), expression, pushlex("}", "switch"), expect("{"), block$1, poplex, poplex);
	if (type == "case") return cont$1(expression, expect(":"));
	if (type == "default") return cont$1(expect(":"));
	if (type == "catch") return cont$1(pushlex("form"), pushcontext, expect("("), funarg, expect(")"), statement, poplex, popcontext);
...
var http_exports = /* @__PURE__ */ __exportAll({ http: () => http });
...
var idl_exports = /* @__PURE__ */ __exportAll({ idl: () => idl });
...
var builtins$6 = wordRegexp$12(builtinArray$1);
var keywordArray$2 = [
	"begin",
	"end",
	"endcase",
...
var keywords$27 = wordRegexp$12(keywordArray$2);
...
function tokenBase$32(stream) {
	...
	if (stream.match(/^'([^']|(''))*'/)) return "string";
	if (stream.match(keywords$27)) return "keyword";
...
var javascript_exports = /* @__PURE__ */ __exportAll({
...
var julia_exports = /* @__PURE__ */ __exportAll({ julia: () => julia });
function wordRegexp$11(words, end, pre) {
...
var openers = wordRegexp$11(openersList);
var closers = wordRegexp$11(closersList);
var keywords$26 = wordRegexp$11(keywordsList);
var builtins$5 = wordRegexp$11(builtinsList);
var macro = /^@[_A-Za-z\u00A1-\uFFFF][\w\u00A1-\uFFFF]*!*/;
var symbol = /^:[_A-Za-z\u00A1-\uFFFF][\w\u00A1-\uFFFF]*!*/;
var stringPrefixes$1 = /^(`|([_A-Za-z\u00A1-\uFFFF]*"("")?))/;
var macroOperators = wordRegexp$11(asciiOperatorsList, "", "@");
var symbolOperators = wordRegexp$11(asciiOperatorsList, "", ":");
...
function tokenBase$31(stream, state) {
	...
	if (stream.match(keywords$26)) return "keyword";
...
function tokenAnnotation(stream, state) {
	...
	else if (state.nestedParameters == 0) state.tokenize = tokenBase$31;
	return "builtin";
}
function tokenComment$13(stream, state) {
	...
	if (stream.match("=#")) {
		...
		if (state.nestedComments == 0) state.tokenize = tokenBase$31;
	}
	return "comment";
}
function tokenChar(stream, state) {
	...
	if (isChar) {
		state.leavingExpr = true;
		state.tokenize = tokenBase$31;
		return "string";
	}
	if (!stream.match(/^[^']+(?=')/)) stream.skipToEnd();
	if (stream.match("'")) state.tokenize = tokenBase$31;
	return "error";
}
...
var livescript_exports = /* @__PURE__ */ __exportAll({ liveScript: () => liveScript });
var tokenBase$30 = function(stream, state) {
...
var liveScript = {
	...
	token: function(stream, state) {
		while (stream.pos == stream.start) var style = tokenBase$30(stream, state);
		state.lastToken = {
			style,
...
var lua_exports = /* @__PURE__ */ __exportAll({ lua: () => lua });
...
var lua = {
	...
	token: function(stream, state) {
		...
		var word = stream.current();
		if (style == "variable") {
			if (keywords$25.test(word)) style = "keyword";
...
var mathematica_exports = /* @__PURE__ */ __exportAll({ mathematica: () => mathematica });
...
function tokenBase$29(stream, state) {
	var ch = stream.next();
	if (ch === "\"") {
		state.tokenize = tokenString$20;
		return state.tokenize(stream, state);
	}
	if (ch === "(") {
		if (stream.eat("*")) {
			state.commentLevel++;
			state.tokenize = tokenComment$12;
			return state.tokenize(stream, state);
		}
	}
	stream.backUp(1);
...
function tokenString$20(stream, state) {
	...
	if (end && !escaped) state.tokenize = tokenBase$29;
	return "string";
}
function tokenComment$12(stream, state) {
	...
	if (state.commentLevel <= 0) state.tokenize = tokenBase$29;
	return "comment";
}
var mathematica = {
	name: "mathematica",
...
var mbox_exports = /* @__PURE__ */ __exportAll({ mbox: () => mbox });
var rfc2822 = [
	"From",
	"Sender",
	"Reply-To",
...
var mirc_exports = /* @__PURE__ */ __exportAll({ mirc: () => mirc });
...
function tokenBase$28(stream, state) {
	...
	if (/[\[\]{}\(\),\.]/.test(ch)) {
		...
		stream.eat(/./);
		return "number";
	} else if (ch == "/" && stream.eat("*")) return chain$6(stream, state, tokenComment$11);
	else if (ch == ";" && stream.match(/ *\( *\(/)) return chain$6(stream, state, tokenUnparsed$2);
		...
		stream.eatWhile(/[\w\$_{}]/);
		var word = stream.current().toLowerCase();
		if (keywords$24 && keywords$24.propertyIsEnumerable(word)) return "keyword";
...
var mllike_exports = /* @__PURE__ */ __exportAll({
	fSharp: () => fSharp,
	oCaml: () => oCaml,
	sml: () => sml
});
...
var modelica_exports = /* @__PURE__ */ __exportAll({ modelica: () => modelica });
function words$13(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var keywords$23 = words$13("algorithm and annotation assert block break class connect connector constant constrainedby der discrete each else elseif elsewhen encapsulated end enumeration equation expandable extends external false final flow for function if import impure in initial inner input loop model not operator or outer output package parameter partial protected public pure record redeclare replaceable return stream then true type when while within");
var builtin$1 = words$13("abs acos actualStream asin atan atan2 cardinality ceil cos cosh delay div edge exp floor getInstanceName homotopy inStream integer log log10 mod pre reinit rem semiLinear sign sin sinh spatialDistribution sqrt tan tanh");
var atoms$7 = words$13("Real Boolean Integer String");
var completions = [].concat(Object.keys(keywords$23), Object.keys(builtin$1), Object.keys(atoms$7));
...
function tokenString$19(stream, state) {
	var escaped = false, ch;
	while ((ch = stream.next()) != null) {
		if (ch == "\"" && !escaped) {
			state.tokenize = null;
...
function tokenIdent(stream, state) {
	...
	state.tokenize = null;
	state.sol = false;
	if (keywords$23.propertyIsEnumerable(cur)) return "keyword";
...
var modelica = {
	...
	token: function(stream, state) {
		...
		else if (ch == "\"") state.tokenize = tokenString$19;
...
var mscgen_exports = /* @__PURE__ */ __exportAll({
	mscgen: () => mscgen,
	msgenny: () => msgenny,
	xu: () => xu
});
...
function produceTokenFunction(pConfig) {
	return function(pStream, pState) {
		if (pStream.match(wordRegexp$10(pConfig.brackets), true, true)) return "bracket";
		if (!pState.inComment) {
			if (pStream.match(/\/\*[^\*\/]*/, true, true)) {
				pState.inComment = true;
				return "comment";
			}
			if (pStream.match(wordRegexp$10(pConfig.singlecomment), true, true)) {
				pStream.skipToEnd();
				return "comment";
			}
		}
		...
		if (pStream.match(wordRegexp$10(pConfig.arcsOthers), true, true)) return "keyword";
		if (!!pConfig.operators && pStream.match(wordRegexp$10(pConfig.operators), true, true)) return "operator";
		if (!!pConfig.constants && pStream.match(wordRegexp$10(pConfig.constants), true, true)) return "variable";
...
var mumps_exports = /* @__PURE__ */ __exportAll({ mumps: () => mumps });
...
function tokenBase$27(stream, state) {
	if (stream.sol()) {
		state.label = true;
		state.commandMode = 0;
	}
...
var mumps = {
	...
	token: function(stream, state) {
		var style = tokenBase$27(stream, state);
...
function tokenBase$26(stream, state) {
	stream.eatWhile(/[\w\$_]/);
	var cur = stream.current();
	if (keywords$22.propertyIsEnumerable(cur)) return "keyword";
	...
	if (ch == "@") {
		...
		state.tokenize = tokenString$18(ch);
		return state.tokenize(stream, state);
	} else if (ch == "#") {
		stream.skipToEnd();
		return ret("comment", "comment");
...
function tokenString$18(quote) {
	return function(stream, state) {
		...
		if (!escaped) state.tokenize = tokenBase$26;
		return ret("string", "string");
	};
}
var nginx = {
...
var nsis_exports = /* @__PURE__ */ __exportAll({ nsis: () => nsis });
var nsis = simpleMode({
	start: [
		{
...
var ntriples_exports = /* @__PURE__ */ __exportAll({ ntriples: () => ntriples });
var Location = {
	PRE_SUBJECT: 0,
	WRITING_SUB_URI: 1,
	WRITING_BNODE_URI: 2,
...
var octave_exports = /* @__PURE__ */ __exportAll({ octave: () => octave });
...
function tokenTranspose(stream, state) {
	if (!stream.sol() && stream.peek() === "'") {
		stream.next();
		state.tokenize = tokenBase$25;
		return "operator";
	}
	state.tokenize = tokenBase$25;
	return tokenBase$25(stream, state);
}
...
function tokenBase$25(stream, state) {
	...
	if (stream.match(keywords$21)) return "keyword";
...
var oz_exports = /* @__PURE__ */ __exportAll({ oz: () => oz });
...
var commonKeywords$4 = wordRegexp$7([
	"andthen",
	"at",
	"attr",
	"declare",
...
function tokenBase$24(stream, state) {
	...
	if (stream.match(middleKeywords$1) || stream.match(commonKeywords$4)) return "keyword";
	...
	var ch = stream.next();
	if (ch == "\"" || ch == "'") {
		state.tokenize = tokenString$17(ch);
		return state.tokenize(stream, state);
	}
	if (/[~\d]/.test(ch)) {
		if (ch == "~") {
...
function tokenString$17(quote) {
	return function(stream, state) {
		...
		if (end || !escaped) state.tokenize = tokenBase$24;
		return "string";
	};
}
...
var pascal_exports = /* @__PURE__ */ __exportAll({ pascal: () => pascal });
function words$11(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var keywords$20 = words$11("absolute and array asm begin case const constructor destructor div do downto else end file for function goto if implementation in inherited inline interface label mod nil not object of operator or packed procedure program record reintroduce repeat self set shl shr string then to type unit until uses var while with xor as class dispinterface except exports finalization finally initialization inline is library on out packed property raise resourcestring threadvar try absolute abstract alias assembler bitpacked break cdecl continue cppdecl cvar default deprecated dynamic enumerator experimental export external far far16 forward generic helper implements index interrupt iocheck local message name near nodefault noreturn nostackframe oldfpccall otherwise overload override pascal platform private protected public published read register reintroduce result safecall saveregisters softfloat specialize static stdcall stored strict unaligned unimplemented varargs virtual write");
...
function tokenBase$23(stream, state) {
	...
	stream.eatWhile(/[\w\$_]/);
	var cur = stream.current().toLowerCase();
	if (keywords$20.propertyIsEnumerable(cur)) return "keyword";
...
var pascal = {
	...
	token: function(stream, state) {
		if (stream.eatSpace()) return null;
		var style = (state.tokenize || tokenBase$23)(stream, state);
		if (style == "comment" || style == "meta") return style;
		return style;
	},
	languageData: {
...
var perl_exports = /* @__PURE__ */ __exportAll({ perl: () => perl });
...
var pig_exports = /* @__PURE__ */ __exportAll({ pig: () => pig });
...
var builtins$2 = words$10(pBuiltins), keywords$19 = words$10(pKeywords), types$4 = words$10(pTypes);
...
function tokenBase$22(stream, state) {
	var ch = stream.next();
	if (ch == "\"" || ch == "'") return chain$5(stream, state, tokenString$15(ch));
	else if (/[\[\]{}\(\),;\.]/.test(ch)) return null;
	else if (/\d/.test(ch)) {
		stream.eatWhile(/[\w\.]/);
		return "number";
		...
		stream.eatWhile(/[\w\$_]/);
		if (keywords$19 && keywords$19.propertyIsEnumerable(stream.current().toUpperCase())) {
...
var powershell_exports = /* @__PURE__ */ __exportAll({ powerShell: () => powerShell });
...
function tokenMultiString(stream, state) {
	...
	if (stream.sol() && stream.match(new RegExp(quote + "@"))) state.tokenize = tokenBase$21;
...
var properties_exports = /* @__PURE__ */ __exportAll({ properties: () => properties });
var properties = {
	name: "properties",
	token: function(stream, state) {
...
var protobuf_exports = /* @__PURE__ */ __exportAll({ protobuf: () => protobuf });
...
var keywords$18 = wordRegexp$6(keywordArray$1);
var identifiers$2 = /* @__PURE__ */ new RegExp("^[_A-Za-z¡-￿][_A-Za-z0-9¡-￿]*");
function tokenBase$20(stream) {
	if (stream.eatSpace()) return null;
	if (stream.match("//")) {
		stream.skipToEnd();
		return "comment";
	...
	if (stream.match(/^"([^"]|(""))*"/)) return "string";
	if (stream.match(/^'([^']|(''))*'/)) return "string";
	if (stream.match(keywords$18)) return "keyword";
...
var pug_exports = /* @__PURE__ */ __exportAll({ pug: () => pug });
var ATTRS_NEST = {
	"{": "}",
	"(": ")",
	"[": "]"
...
var puppet_exports = /* @__PURE__ */ __exportAll({ puppet: () => puppet });
var words$9 = {};
var variable_regex = /({)?([a-z][a-z0-9_]*)?((::[a-z][a-z0-9_]*)*::)?[a-zA-Z0-9_]+(})?/;
function define$2(style, string) {
	var split = string.split(" ");
	for (var i = 0; i < split.length; i++) words$9[split[i]] = style;
}
...
function tokenString$14(stream, state) {
	var current, prev, found_var = false;
	while (!stream.eol() && (current = stream.next()) != state.pending) {
		if (current === "$" && prev != "\\" && state.pending == "\"") {
			found_var = true;
...
function tokenize$3(stream, state) {
	...
	if (state.continueString) {
		stream.backUp(1);
		return tokenString$14(stream, state);
	}
	...
	if (word && words$9.hasOwnProperty(word)) {
		stream.backUp(1);
		stream.match(/[\w]+/);
...
var wordOperators$1 = wordRegexp$5([
	"and",
	"or",
	"not",
	"is"
]);
var commonKeywords$3 = [
	"as",
	"assert",
	"break",
	"class",
...
function mkPython(parserConf) {
	...
	var myKeywords = commonKeywords$3, myBuiltins = commonBuiltins$1;
	...
	var keywords = wordRegexp$5(myKeywords);
	var builtins = wordRegexp$5(myBuiltins);
	function tokenBase(stream, state) {
	...
	return {
		...
		languageData: {
			autocomplete: commonKeywords$3.concat(commonBuiltins$1).concat(["exec", "print"]),
...
var words$8 = function(str) {
	return str.split(" ");
};
mkPython({});
var cython = mkPython({ extra_keywords: words$8("by cdef cimport cpdef ctypedef enum except extern gil include nogil property public readonly struct union DEF IF ELIF ELSE") });
//#endregion
//#region node_modules/@codemirror/legacy-modes/mode/q.js
var q_exports = /* @__PURE__ */ __exportAll({ q: () => q });
var curPunc$7, keywords$17 = buildRE([
	"abs",
	"acos",
	"aj",
	"aj0",
...
function tokenBase$19(stream, state) {
	...
	if (/[A-Za-z]|\./.test(c)) return stream.eatWhile(/[A-Za-z._\d]/), keywords$17.test(stream.current()) ? "keyword" : "variable";
	if (/[|/&^!+:\\\-*%$=~#;@><\.,?_\']/.test(c)) return null;
	if (/[{}\(\[\]\)]/.test(c)) return null;
	return "error";
}
function tokenLineComment(stream, state) {
	return stream.skipToEnd(), /^\/\s*$/.test(stream.current()) ? (state.tokenize = tokenBlockComment)(stream, state) : state.tokenize = tokenBase$19, "comment";
}
function tokenBlockComment(stream, state) {
	var f = stream.sol() && stream.peek() == "\\";
	stream.skipToEnd();
	if (f && /^\\\s*$/.test(stream.current())) state.tokenize = tokenBase$19;
	return "comment";
}
...
function tokenString$13(stream, state) {
	...
	if (end) state.tokenize = tokenBase$19;
	return "string";
}
function pushContext$8(state, type, col) {
	state.context = {
		prev: state.context,
		indent: state.indent,
		col,
...
var q = {
	...
	token: function(stream, state) {
		...
		if (curPunc$7 == "(") pushContext$8(state, ")", stream.column());
			...
			if (/[\}\]]/.test(state.context.type)) pushContext$8(state, "pattern", stream.column());
			else if (state.context.type == "pattern" && !state.context.align) {
				state.context.align = true;
				state.context.col = stream.column();
			}
...
var r_exports = /* @__PURE__ */ __exportAll({ r: () => r });
...
var keywords$16 = wordObj$1(commonKeywords$2);
var blockkeywords = wordObj$1(commonBlockKeywords);
var opChars = /[+\-*\/^<>=!&|~$:]/;
var curPunc$6;
function tokenBase$18(stream, state) {
	curPunc$6 = null;
	var ch = stream.next();
	if (ch == "#") {
		stream.skipToEnd();
		...
		state.tokenize = tokenString$12(ch);
		return "string";
	} else if (ch == "`") {
		stream.match(/[^`]+`/);
		return "string.special";
		...
		if (keywords$16.propertyIsEnumerable(word)) {
...
var r = {
	...
	indent: function(state, textAfter, cx) {
		if (state.tokenize != tokenBase$18) return 0;
...
var rpm_exports = /* @__PURE__ */ __exportAll({
...
var ruby_exports = /* @__PURE__ */ __exportAll({ ruby: () => ruby });
...
function tokenBase$17(stream, state) {
	...
	if (ch == "`" || ch == "'" || ch == "\"") return chain$4(readQuoted(ch, "string", ch == "\"" || ch == "`"), stream, state);
		...
		stream.skipToEnd();
		return "comment";
	} else if (ch == "<" && (m = stream.match(/^<([-~])[\`\"\']?([a-zA-Z_?]\w*)[\`\"\']?(?:;|$)/))) return chain$4(readHereDoc(m[2], m[1]), stream, state);
	else if (ch == "0") {
...
var ruby = {
	...
	token: function(stream, state) {
		...
		if (style == "variable") {
			var word = stream.current();
			style = state.lastTok == "." ? "property" : keywords$15.propertyIsEnumerable(stream.current()) ? "keyword" : /^[A-Z]/.test(word) ? "tag" : state.lastTok == "def" || state.lastTok == "class" || state.varList ? "def" : "variable";
			if (style == "keyword") {
				thisTok = word;
	...
	indent: function(state, textAfter, cx) {
		if (state.tokenize[state.tokenize.length - 1] != tokenBase$17) return null;
		var firstChar = textAfter && textAfter.charAt(0);
...
var sas_exports = /* @__PURE__ */ __exportAll({ sas: () => sas });
var words$7 = {};
...
var scheme_exports = /* @__PURE__ */ __exportAll({ scheme: () => scheme });
			...
			default:
				...
				if (ch == "\"") {
					...
					*/
					...
					stream.eatWhile(/[\w_\-!$%&*+\.\/:<=>?@\^~]/);
					if (keywords$14 && keywords$14.propertyIsEnumerable(stream.current())) returnType = BUILTIN;
...
var shell_exports = /* @__PURE__ */ __exportAll({ shell: () => shell });
var words$6 = {};
...
function tokenBase$16(stream, state) {
	...
	if (ch === "'" || ch === "\"" || ch === "`") {
		state.tokens.unshift(tokenString$11(ch, ch === "`" ? "quote" : "string"));
...
function tokenString$11(quote, style) {
	...
	return function(stream, state) {
		...
		while ((next = stream.next()) != null) {
			if (next === close && !escaped) {
				...
				state.tokens.unshift(tokenString$11(quote, style));
				return tokenize$1(stream, state);
...
function tokenStringStart(quote, style) {
	return function(stream, state) {
		state.tokens[0] = tokenString$11(quote, style);
		stream.next();
		return tokenize$1(stream, state);
	};
}
var tokenDollar = function(stream, state) {
	...
	var ch = stream.next();
	if (/['"({]/.test(ch)) {
		state.tokens[0] = tokenString$11(ch, ch == "(" ? "quote" : ch == "{" ? "def" : "string");
		return tokenize$1(stream, state);
	}
...
function tokenize$1(stream, state) {
	return (state.tokens[0] || tokenBase$16)(stream, state);
}
var shell = {
	name: "shell",
	startState: function() {
...
var sieve_exports = /* @__PURE__ */ __exportAll({ sieve: () => sieve });
function words$5(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var keywords$13 = words$5("if elsif else stop require");
...
function tokenBase$15(stream, state) {
	...
	if (ch == "\"") {
		state.tokenize = tokenString$10(ch);
		return state.tokenize(stream, state);
	}
	if (ch == "(") {
		state._indent.push("(");
	...
	if (keywords$13.propertyIsEnumerable(cur)) return "keyword";
...
function tokenString$10(quote) {
	return function(stream, state) {
		...
		if (!escaped) state.tokenize = tokenBase$15;
		return "string";
	};
}
var sieve = {
	...
	token: function(stream, state) {
		if (stream.eatSpace()) return null;
		return (state.tokenize || tokenBase$15)(stream, state);
	},
...
var smalltalk_exports = /* @__PURE__ */ __exportAll({ smalltalk: () => smalltalk });
var specialChars = /[+\-\/\\*~<>=@%|&?!.,:;^]/;
var keywords$12 = /true|false|nil|self|super|thisContext/;
...
var next = function(stream, context, state) {
	...
	if (aChar === "\"") token = nextComment(stream, new Context$5(nextComment, context));
		...
		stream.eatWhile(/[\w\d_]/);
		token.name = state.expectVariable ? keywords$12.test(stream.current()) ? "keyword" : "variable" : null;
...
var solr_exports = /* @__PURE__ */ __exportAll({ solr: () => solr });
var isStringChar = /[^\s\|\!\+\-\*\?\~\^\&\:\(\)\[\]\{\}\"\\]/;
...
function tokenString$9(quote) {
	return function(stream, state) {
		...
		if (!escaped) state.tokenize = tokenBase$14;
		return "string";
	};
}
...
function tokenBase$14(stream, state) {
	var ch = stream.next();
	if (ch == "\"") state.tokenize = tokenString$9(ch);
	else if (isOperatorChar$5.test(ch)) state.tokenize = tokenOperator(ch);
	else if (isStringChar.test(ch)) state.tokenize = tokenWord(ch);
	return state.tokenize != tokenBase$14 ? state.tokenize(stream, state) : null;
}
var solr = {
	name: "solr",
	startState: function() {
		return { tokenize: tokenBase$14 };
	},
	token: function(stream, state) {
		if (stream.eatSpace()) return null;
		return state.tokenize(stream, state);
...
var sparql_exports = /* @__PURE__ */ __exportAll({ sparql: () => sparql });
var curPunc$4;
function wordRegexp$4(words) {
	return new RegExp("^(?:" + words.join("|") + ")$", "i");
}
var ops$1 = wordRegexp$4([
	"str",
	"lang",
	"langmatches",
	"datatype",
	...
	token: function(stream, state) {
		...
		if (curPunc$4 == "(") pushContext$7(state, ")", stream.column());
			...
			if (state.context && curPunc$4 == state.context.type) {
				popContext$7(state);
				if (curPunc$4 == "}" && state.context && state.context.type == "pattern") popContext$7(state);
			}
...
var spreadsheet_exports = /* @__PURE__ */ __exportAll({ spreadsheet: () => spreadsheet });
var spreadsheet = {
	name: "spreadsheet",
	startState: function() {
		return {
...
var stex_exports = /* @__PURE__ */ __exportAll({ stex: () => stex });
...
var stylus_exports = /* @__PURE__ */ __exportAll({ stylus: () => stylus });
var tagKeywords_ = [
	"a",
	"abbr",
	"address",
...
*/
states.block = function(type, stream, state) {
	if (type == "comment" && startOfLine(stream) || type == "," && endOfLine(stream) || type == "mixin") return pushContext$6(state, stream, "block", 0);
	if (typeIsInterpolation(type, stream)) return pushContext$6(state, stream, "interpolation");
	if (endOfLine(stream) && type == "]") {
		if (!/^\s*(\.|#|:|\[|\*|&)/.test(stream.string) && !wordIsTag(firstWordOfLine(stream))) return pushContext$6(state, stream, "block", 0);
	}
	...
	if (typeIsPseudo(type, stream)) return pushContext$6(state, stream, "pseudo");
	if (/@(font-face|media|supports|(-moz-)?document)/.test(type)) return pushContext$6(state, stream, endOfLine(stream) ? "block" : "atBlock");
	if (/@(-(moz|ms|o|webkit)-)?keyframes$/.test(type)) return pushContext$6(state, stream, "keyframes");
	if (/@extends?/.test(type)) return pushContext$6(state, stream, "extend", 0);
	if (type && type.charAt(0) == "@") {
		if (stream.indentation() > 0 && wordIsProperty(stream.current().slice(1))) {
			override = "variable";
			return "block";
		}
		if (/(@import|@require|@charset)/.test(type)) return pushContext$6(state, stream, "block", 0);
		return pushContext$6(state, stream, "block");
	}
	if (type == "reference" && endOfLine(stream)) return pushContext$6(state, stream, "block");
	if (type == "(") return pushContext$6(state, stream, "parens");
	if (type == "vendor-prefixes") return pushContext$6(state, stream, "vendorPrefixes");
	if (type == "word") {
		var word = stream.current();
		...
		if (override == "variable" && stream.string.match(/^\s?\$[\w-\.\[\]\'\"]+$/)) return pushContext$6(state, stream, "block");
	}
	return state.context.type;
};
/**
...
states.parens = function(type, stream, state) {
	if (type == "(") return pushContext$6(state, stream, "parens");
	if (type == ")") {
		if (state.context.prev.type == "parens") return popContext$6(state, stream);
		if (stream.string.match(/^[a-z][\w-]*\(/i) && endOfLine(stream) || wordIsBlock(firstWordOfLine(stream)) || /(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(firstWordOfLine(stream)) || !stream.string.match(/^-?[a-z][\w-\.\[\]\'\"]*\s*=/) && wordIsTag(firstWordOfLine(stream))) return pushContext$6(state, stream, "block");
		if (stream.string.match(/^[\$-]?[a-z][\w-\.\[\]\'\"]*\s*=/) || stream.string.match(/^\s*(\(|\)|[0-9])/) || stream.string.match(/^\s+[a-z][\w-]*\(/i) || stream.string.match(/^\s+[\$-]?[a-z]/i)) return pushContext$6(state, stream, "block", 0);
		if (endOfLine(stream)) return pushContext$6(state, stream, "block");
	...
	if (type == "variable-name") return pushContext$6(state, stream, "variableName");
	if (typeIsPseudo(type, stream)) return pushContext$6(state, stream, "pseudo");
	return state.context.type;
};
/**
...
states.atBlock = function(type, stream, state) {
	...
	if (type == "word") {
		...
		if (override == "tag" && endOfLine(stream)) return pushContext$6(state, stream, "block");
	}
...
states.keyframes = function(type, stream, state) {
	...
	if (/@(font-face|media|supports|(-moz-)?document)/.test(type)) return pushContext$6(state, stream, endOfLine(stream) ? "block" : "atBlock");
	if (type == "mixin") return pushContext$6(state, stream, "block", 0);
	return state.context.type;
};
/**
* Interpolation
...
states.interpolation = function(type, stream, state) {
	...
	if (type == "}") {
		if (stream.string.match(/^\s*(\.|#|:|\[|\*|&|>|~|\+|\/)/i) || stream.string.match(/^\s*[a-z]/i) && wordIsTag(firstWordOfLine(stream))) return pushContext$6(state, stream, "block");
		if (!stream.string.match(/^(\{|\s*\&)/) || stream.match(/\s*[\w-]/, false)) return pushContext$6(state, stream, "block", 0);
		return pushContext$6(state, stream, "block");
	}
	if (type == "variable-name") return pushContext$6(state, stream, "variableName", 0);
	if (type == "word") {
		override = wordAsValue(stream.current());
...
var stylus = {
	...
	token: function(stream, state) {
		if (!state.tokenize && stream.eatSpace()) return null;
		style = (state.tokenize || tokenBase$12)(stream, state);
		if (style && typeof style == "object") {
			type$1 = style[1];
			style = style[0];
		}
...
var swift_exports = /* @__PURE__ */ __exportAll({ swift: () => swift });
function wordSet(words) {
	var set = {};
	for (var i = 0; i < words.length; i++) set[words[i]] = true;
	return set;
}
var keywords$10 = wordSet([
	"_",
	"var",
	"let",
	"actor",
...
function tokenBase$11(stream, state, prev) {
	if (stream.sol()) state.indented = stream.indentation();
	if (stream.eatSpace()) return null;
	var ch = stream.peek();
	if (ch == "/") {
	...
	if (stream.match(identifier)) {
		...
		if (keywords$10.hasOwnProperty(ident)) {
...
var swift = {
	...
	token: function(stream, state) {
		var prev = state.prev;
		state.prev = null;
		var style = (state.tokenize[state.tokenize.length - 1] || tokenBase$11)(stream, state, prev);
...
var tcl_exports = /* @__PURE__ */ __exportAll({ tcl: () => tcl });
...
function tokenBase$10(stream, state) {
	...
	state.beforeParams = false;
	var ch = stream.next();
	if ((ch == "\"" || ch == "'") && state.inParams) return chain$3(stream, state, tokenString$6(ch));
	else if (/[\[\]{}\(\),;\.]/.test(ch)) {
		if (ch == "(" && beforeParams) state.inParams = true;
		else if (ch == ")") state.inParams = false;
		return null;
		...
		if (stream.eat("*")) return chain$3(stream, state, tokenComment$4);
		if (ch == "#" && stream.match(/ *\[ *\[/)) return chain$3(stream, state, tokenUnparsed$1);
		stream.skipToEnd();
		return "comment";
	} else if (ch == "\"") {
		stream.skipTo(/"/);
		...
		stream.eatWhile(/[\w\$_{}\xa1-\uffff]/);
		var word = stream.current().toLowerCase();
		if (keywords$9 && keywords$9.propertyIsEnumerable(word)) return "keyword";
...
function tokenString$6(quote) {
	return function(stream, state) {
		...
		if (end) state.tokenize = tokenBase$10;
		return "string";
	};
}
...
var textile_exports = /* @__PURE__ */ __exportAll({ textile: () => textile });
var TOKEN_STYLES = {
	addition: "inserted",
...
var tiddlywiki_exports = /* @__PURE__ */ __exportAll({ tiddlyWiki: () => tiddlyWiki });
var textwords = {};
var keywords$8 = {
	"allTags": true,
	"closeAll": true,
...
function tokenBase$9(stream, state) {
	...
	if (ch == "{" && stream.match("{{")) return chain$2(stream, state, twTokenCode);
	...
	if (ch == "/") {
		if (stream.eat("%")) return chain$2(stream, state, twTokenComment);
		else if (stream.eat("/")) return chain$2(stream, state, twTokenEm);
	}
	if (ch == "_" && stream.eat("_")) return chain$2(stream, state, twTokenUnderline);
	if (ch == "-" && stream.eat("-")) {
		if (stream.peek() != " ") return chain$2(stream, state, twTokenStrike);
		if (stream.peek() == " ") return "brace";
	}
	if (ch == "'" && stream.eat("'")) return chain$2(stream, state, twTokenStrong);
	if (ch == "<" && stream.eat("<")) return chain$2(stream, state, twTokenMacro);
	stream.eatWhile(/[\w\$_]/);
...
var tiki_exports = /* @__PURE__ */ __exportAll({ tiki: () => tiki });
...
function endplugin(startOfLine) {
	return function(type) {
		...
		if (type == "endPlugin") {
			pushContext$4(curState.pluginName, startOfLine);
			return cont();
		}
		return cont();
	};
...
var toml_exports = /* @__PURE__ */ __exportAll({ toml: () => toml });
var toml = {
	name: "toml",
	startState: function() {
		return {
...
var troff_exports = /* @__PURE__ */ __exportAll({ troff: () => troff });
var words$4 = {};
...
var ttcn_cfg_exports = /* @__PURE__ */ __exportAll({ ttcnCfg: () => ttcnCfg });
...
var parserConfig$1 = {
	name: "ttcn-cfg",
	keywords: words$3("Yes No LogFile FileMask ConsoleMask AppendFile TimeStampFormat LogEventTypes SourceInfoFormat LogEntityName LogSourceInfo DiskFullAction LogFileNumber LogFileSize MatchingHints Detailed Compact SubCategories Stack Single None Seconds DateTime Time Stop Error Retry Delete TCPPort KillTimer NumHCs UnixSocketsEnabled LocalAddress"),
	fileNCtrlMaskOptions: words$3("TTCN_EXECUTOR TTCN_ERROR TTCN_WARNING TTCN_PORTEVENT TTCN_TIMEROP TTCN_VERDICTOP TTCN_DEFAULTOP TTCN_TESTCASE TTCN_ACTION TTCN_USER TTCN_FUNCTION TTCN_STATISTICS TTCN_PARALLEL TTCN_MATCHING TTCN_DEBUG EXECUTOR ERROR WARNING PORTEVENT TIMEROP VERDICTOP DEFAULTOP TESTCASE ACTION USER FUNCTION STATISTICS PARALLEL MATCHING DEBUG LOG_ALL LOG_NOTHING ACTION_UNQUALIFIED DEBUG_ENCDEC DEBUG_TESTPORT DEBUG_UNQUALIFIED DEFAULTOP_ACTIVATE DEFAULTOP_DEACTIVATE DEFAULTOP_EXIT DEFAULTOP_UNQUALIFIED ERROR_UNQUALIFIED EXECUTOR_COMPONENT EXECUTOR_CONFIGDATA EXECUTOR_EXTCOMMAND EXECUTOR_LOGOPTIONS EXECUTOR_RUNTIME EXECUTOR_UNQUALIFIED FUNCTION_RND FUNCTION_UNQUALIFIED MATCHING_DONE MATCHING_MCSUCCESS MATCHING_MCUNSUCC MATCHING_MMSUCCESS MATCHING_MMUNSUCC MATCHING_PCSUCCESS MATCHING_PCUNSUCC MATCHING_PMSUCCESS MATCHING_PMUNSUCC MATCHING_PROBLEM MATCHING_TIMEOUT MATCHING_UNQUALIFIED PARALLEL_PORTCONN PARALLEL_PORTMAP PARALLEL_PTC PARALLEL_UNQUALIFIED PORTEVENT_DUALRECV PORTEVENT_DUALSEND PORTEVENT_MCRECV PORTEVENT_MCSEND PORTEVENT_MMRECV PORTEVENT_MMSEND PORTEVENT_MQUEUE PORTEVENT_PCIN PORTEVENT_PCOUT PORTEVENT_PMIN PORTEVENT_PMOUT PORTEVENT_PQUEUE PORTEVENT_STATE PORTEVENT_UNQUALIFIED STATISTICS_UNQUALIFIED STATISTICS_VERDICT TESTCASE_FINISH TESTCASE_START TESTCASE_UNQUALIFIED TIMEROP_GUARD TIMEROP_READ TIMEROP_START TIMEROP_STOP TIMEROP_TIMEOUT TIMEROP_UNQUALIFIED USER_UNQUALIFIED VERDICTOP_FINAL VERDICTOP_GETVERDICT VERDICTOP_SETVERDICT VERDICTOP_UNQUALIFIED WARNING_UNQUALIFIED"),
	externalCommands: words$3("BeginControlPart EndControlPart BeginTestCase EndTestCase"),
	multiLineStrings: true
};
var keywords$7 = parserConfig$1.keywords, fileNCtrlMaskOptions = parserConfig$1.fileNCtrlMaskOptions, externalCommands = parserConfig$1.externalCommands, multiLineStrings$2 = parserConfig$1.multiLineStrings, indentStatements$1 = parserConfig$1.indentStatements !== false;
...
var ttcn_exports = /* @__PURE__ */ __exportAll({ ttcn: () => ttcn });
...
var parserConfig = {
	name: "ttcn",
	keywords: words$2("activate address alive all alt altstep and and4b any break case component const continue control deactivate display do else encode enumerated except exception execute extends extension external for from function goto group if import in infinity inout interleave label language length log match message mixed mod modifies module modulepar mtc noblock not not4b nowait of on optional or or4b out override param pattern port procedure record recursive rem repeat return runs select self sender set signature system template testcase to type union value valueof var variant while with xor xor4b"),
	builtin: words$2("bit2hex bit2int bit2oct bit2str char2int char2oct encvalue decomp decvalue float2int float2str hex2bit hex2int hex2oct hex2str int2bit int2char int2float int2hex int2oct int2str int2unichar isbound ischosen ispresent isvalue lengthof log2str oct2bit oct2char oct2hex oct2int oct2str regexp replace rnd sizeof str2bit str2float str2hex str2int str2oct substr unichar2int unichar2char enum2int"),
	types: words$2("anytype bitstring boolean char charstring default float hexstring integer objid octetstring universal verdicttype timer"),
	timerOps: words$2("read running start stop timeout"),
	portOps: words$2("call catch check clear getcall getreply halt raise receive reply send trigger"),
	configOps: words$2("create connect disconnect done kill killed map unmap"),
	verdictOps: words$2("getverdict setverdict"),
	sutOps: words$2("action"),
	functionOps: words$2("apply derefers refers"),
	verdictConsts: words$2("error fail inconc none pass"),
	booleanConsts: words$2("true false"),
	otherConsts: words$2("null NULL omit"),
	visibilityModifiers: words$2("private public friend"),
	templateMatch: words$2("complement ifpresent subset superset permutation"),
	multiLineStrings: true
};
var wordList = [];
function add(obj) {
...
var keywords$6 = parserConfig.keywords || {}, builtin = parserConfig.builtin || {}, timerOps = parserConfig.timerOps || {}, portOps = parserConfig.portOps || {}, configOps = parserConfig.configOps || {}, verdictOps = parserConfig.verdictOps || {}, sutOps = parserConfig.sutOps || {}, functionOps = parserConfig.functionOps || {}, verdictConsts = parserConfig.verdictConsts || {}, booleanConsts = parserConfig.booleanConsts || {}, otherConsts = parserConfig.otherConsts || {}, types$2 = parserConfig.types || {}, visibilityModifiers = parserConfig.visibilityModifiers || {}, templateMatch = parserConfig.templateMatch || {}, multiLineStrings$1 = parserConfig.multiLineStrings, indentStatements = parserConfig.indentStatements !== false;
...
var turtle_exports = /* @__PURE__ */ __exportAll({ turtle: () => turtle });
var curPunc$1;
function wordRegexp$2(words) {
	return new RegExp("^(?:" + words.join("|") + ")$", "i");
}
var ops = wordRegexp$2([]);
var keywords$5 = wordRegexp$2([
	"@prefix",
	"@base",
	"a"
]);
...
var vb_exports = /* @__PURE__ */ __exportAll({ vb: () => vb });
var ERRORCLASS = "error";
function wordRegexp$1(words) {
	return new RegExp("^((" + words.join(")|(") + "))\\b", "i");
}
var singleOperators$1 = /* @__PURE__ */ new RegExp("^[\\+\\-\\*/%&\\\\|\\^~<>!]");
...
var identifiers$1 = /* @__PURE__ */ new RegExp("^[_A-Za-z][_A-Za-z0-9]*");
var openingKeywords = [
	"class",
	"module",
	"sub",
...
var keywords$4 = wordRegexp$1(commonKeywords);
var types$1 = wordRegexp$1(commontypes);
var stringPrefixes = "\"";
...
function tokenBase$4(stream, state) {
	...
	if (stream.match(doubleOperators) || stream.match(singleOperators$1) || stream.match(wordOperators)) return "operator";
	...
	if (stream.match(types$1)) return "keyword";
...
var vbscript_exports = /* @__PURE__ */ __exportAll({ vbScript: () => vbScript });
...
var velocity_exports = /* @__PURE__ */ __exportAll({ velocity: () => velocity });
function parseWords(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
}
var keywords$3 = parseWords("#end #else #break #stop #[[ #]] #{end} #{else} #{break} #{stop}");
...
function tokenBase$3(stream, state) {
	...
	if (ch == "'" && !state.inString && state.inParams) {
		...
		stream.eatWhile(/[\w\$_{}@]/);
		var word = stream.current();
		if (keywords$3 && keywords$3.propertyIsEnumerable(word)) return "keyword";
...
var verilog_exports = /* @__PURE__ */ __exportAll({ verilog: () => verilog });
...
var vhdl_exports = /* @__PURE__ */ __exportAll({ vhdl: () => vhdl });
...
var atoms$1 = words$1("null"), hooks = {
	"`": metaHook,
	"$": metaHook
}, multiLineStrings = false;
var keywords$2 = words$1("abs,access,after,alias,all,and,architecture,array,assert,attribute,begin,block,body,buffer,bus,case,component,configuration,constant,disconnect,downto,else,elsif,end,end block,end case,end component,end for,end generate,end if,end loop,end process,end record,end units,entity,exit,file,for,function,generate,generic,generic map,group,guarded,if,impure,in,inertial,inout,is,label,library,linkage,literal,loop,map,mod,nand,new,next,nor,null,of,on,open,or,others,out,package,package body,port,port map,postponed,procedure,process,pure,range,record,register,reject,rem,report,return,rol,ror,select,severity,signal,sla,sll,sra,srl,subtype,then,to,transport,type,unaffected,units,until,use,variable,wait,when,while,with,xnor,xor");
...
function tokenBase$2(stream, state) {
	...
	stream.eatWhile(/[\w\$_]/);
	var cur = stream.current();
	if (keywords$2.propertyIsEnumerable(cur.toLowerCase())) {
...
var webidl_exports = /* @__PURE__ */ __exportAll({ webIDL: () => webIDL });
function wordRegexp(words) {
	return new RegExp("^((" + words.join(")|(") + "))\\b");
}
var builtinArray = [
...
var keywords$1 = wordRegexp(keywordArray);
var atomArray = [
	"true",
	"false",
	"Infinity",
...
var xquery_exports = /* @__PURE__ */ __exportAll({ xQuery: () => xQuery });
var keywords = function() {
	function kw(type) {
		return {
			type,
...
var yacas_exports = /* @__PURE__ */ __exportAll({ yacas: () => yacas });
function words(str) {
	var obj = {}, words = str.split(" ");
	for (var i = 0; i < words.length; ++i) obj[words[i]] = true;
	return obj;
...
var z80_exports = /* @__PURE__ */ __exportAll({ z80: () => z80 });
function mkZ80(ez80) {
...
mkZ80(true);
//#endregion
export { mirc_exports as $, sas_exports as A, commonlisp_exports as At, powershell_exports as B, apl_exports as Bt, spreadsheet_exports as C, dtd_exports as Ct, sieve_exports as D, cypher_exports as Dt, smalltalk_exports as E, d_exports as Et, python_exports as F, clike_exports as Ft, octave_exports as G, perl_exports as H, puppet_exports as I, brainfuck_exports as It, nginx_exports as J, ntriples_exports as K, pug_exports as L, asterisk_exports as Lt, rpm_exports as M, cobol_exports as Mt, r_exports as N, cmake_exports as Nt, shell_exports as O, css_exports as Ot, q_exports as P, clojure_exports as Pt, mllike_exports as Q, protobuf_exports as R, asn1_exports as Rt, sql_exports as S, dylan_exports as St, solr_exports as T, diff_exports as Tt, pascal_exports as U, pig_exports as V, oz_exports as W, mscgen_exports as X, mumps_exports as Y, modelica_exports as Z, textile_exports as _, erlang_exports as _t, vhdl_exports as a, javascript_exports as at, stylus_exports as b, ecl_exports as bt, vbscript_exports as c, haxe_exports as ct, ttcn_exports as d, gherkin_exports as dt, mbox_exports as et, ttcn_cfg_exports as f, gas_exports as ft, tiddlywiki_exports as g, factor_exports as gt, tiki_exports as h, fcl_exports as ht, webidl_exports as i, julia_exports as it, ruby_exports as j, coffeescript_exports as jt, scheme_exports as k, crystal_exports as kt, vb_exports as l, haskell_exports as lt, toml_exports as m, forth_exports as mt, yacas_exports as n, lua_exports as nt, verilog_exports as o, idl_exports as ot, troff_exports as p, fortran_exports as pt, nsis_exports as q, xquery_exports as r, livescript_exports as rt, velocity_exports as s, http_exports as st, z80_exports as t, mathematica_exports as tt, turtle_exports as u, groovy_exports as ut, tcl_exports as v, elm_exports as vt, sparql_exports as w, dockerfile_exports as wt, stex_exports as x, ebnf_exports as xt, swift_exports as y, eiffel_exports as yt, properties_exports as z, asciiarmor_exports as zt };
