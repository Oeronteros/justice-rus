import { i as formatCodeAsIndented, m as decodeString, n as formatHeadingAsSetext, o as patternInScope, p as remarkParse, r as encodeCharacterReference, s as unified, t as handle } from "./handle-CTtDxEnq.js";
import { n as keyName, t as base } from "./w3c-keyname-8losroVz.js";
...
var Slice$1 = class Slice$1 {
	...
	*/
	removeBetween(from, to) {
		return new Slice$1(removeRange(this.content, from + this.openStart, to + this.openStart), this.openStart, this.openEnd);
	}
	/**
	...
	*/
	static fromJSON(schema, json) {
		...
		return new Slice$1(Fragment.fromJSON(schema, json.content), openStart, openEnd);
	}
	/**
...
The empty slice.
*/
Slice$1.empty = new Slice$1(Fragment.empty, 0, 0);
...
var AddMarkStep = class AddMarkStep extends Step {
	...
	*/
	...
	apply(doc) {
		...
		let slice = new Slice$1(mapFragment(oldSlice.content, (node, parent) => {
...
var AddNodeMarkStep = class AddNodeMarkStep extends Step {
	...
	*/
	...
	apply(doc) {
		...
		return StepResult.fromReplace(doc, this.pos, this.pos + 1, new Slice$1(Fragment.from(updated), 0, node.isLeaf ? 0 : 1));
	}
	invert(doc) {
		let node = doc.nodeAt(this.pos);
		if (node) {
...
function clearIncompatible(tr, pos, parentType, match = parentType.contentMatch, clearNewlines = true) {
	...
	for (let i = 0; i < node.childCount; i++) {
		...
		if (!allowed) replSteps.push(new ReplaceStep(cur, end, Slice$1.empty));
		else {
			match = allowed;
			...
			if (clearNewlines && child.isText && parentType.whitespace != "pre") {
				...
				while (m = newline.exec(child.text)) {
					if (!slice) slice = new Slice$1(Fragment.from(parentType.schema.text(" ", parentType.allowedMarks(child.marks))), 0, 0);
...
function lift(tr, range, target) {
	...
	tr.step(new ReplaceAroundStep(start, end, gapStart, gapEnd, new Slice$1(before.append(after), openStart, openEnd), before.size - openStart, true));
}
/**
...
function wrap(tr, range, wrappers) {
	...
	tr.step(new ReplaceAroundStep(start, end, start, end, new Slice$1(content, 0, 0), wrappers.length, true));
}
function setBlockType$1(tr, from, to, type, attrs) {
	...
	tr.doc.nodesBetween(from, to, (node, pos) => {
		...
		if (node.isTextblock && !node.hasMarkup(type, attrsHere) && canChangeType(tr.doc, tr.mapping.slice(mapFrom).map(pos), type)) {
			...
			tr.step(new ReplaceAroundStep(startM, endM, startM + 1, endM - 1, new Slice$1(Fragment.from(type.create(attrsHere, null, node.marks)), 0, 0), 1, true));
...
function setNodeMarkup(tr, pos, type, attrs, marks) {
	...
	tr.step(new ReplaceAroundStep(pos, pos + node.nodeSize, pos + 1, pos + node.nodeSize - 1, new Slice$1(Fragment.from(newNode), 0, 0), 1, true));
}
/**
...
function split(tr, pos, depth = 1, typesAfter) {
	...
	tr.step(new ReplaceStep(pos, pos, new Slice$1(before.append(after), depth, depth), true));
}
/**
...
function join(tr, pos, depth) {
	...
	tr.step(new ReplaceStep(start, mapping.map(pos + depth, -1), Slice$1.empty, true));
...
var Fitter = class {
	...
	dropNode() {
		...
		} else this.unplaced = new Slice$1(dropFromFragment(content, openStart, 1), openStart, openEnd);
	}
	placeNodes({ sliceDepth, frontierDepth, parent, inject, wrap }) {
		...
		this.unplaced = !toEnd ? new Slice$1(dropFromFragment(slice.content, sliceDepth, taken), slice.openStart, slice.openEnd) : sliceDepth == 0 ? Slice$1.empty : new Slice$1(dropFromFragment(slice.content, sliceDepth - 1, 1), sliceDepth - 1, openEndCount < 0 ? slice.openEnd : sliceDepth - 1);
	}
	mustMoveInline() {
...
function replaceRange$1(tr, from, to, slice) {
	...
	for (let j = slice.openStart; j >= 0; j--) {
		...
		for (let i = 0; i < targetDepths.length; i++) {
			...
			if (parent.canReplaceWith(index, index, insert.type, insert.marks)) return tr.replace($from.before(targetDepth), expand ? $to.after(targetDepth) : to, new Slice$1(closeFragment(slice.content, 0, slice.openStart, openDepth), openDepth, slice.openEnd));
		}
	}
...
function replaceRangeWith(tr, from, to, node) {
	...
	tr.replaceRange(from, to, new Slice$1(Fragment.from(node), 0, 0));
}
...
var Transform = class {
	...
	*/
	replaceWith(from, to, content) {
		return this.replace(from, to, new Slice$1(Fragment.from(content), 0, 0));
	}
	/**
...
var NodeSelection = class NodeSelection extends Selection {
	...
	*/
	...
	content() {
		return new Slice$1(Fragment.from(this.node), 0, 0);
	}
	eq(other) {
...
var ie_version$1 = ie_upto10$1 ? document.documentMode : ie_11up$1 ? +ie_11up$1[1] : ie_edge$1 ? +ie_edge$1[1] : 0;
var gecko$1 = !ie$1 && /gecko\/(\d+)/i.test(agent$1);
var gecko_version = gecko$1 && +(/Firefox\/(\d+)/.exec(agent$1) || [0, 0])[1];
var _chrome$1 = !ie$1 && /Chrome\/(\d+)/.exec(agent$1);
var chrome$1 = !!_chrome$1;
var chrome_version$1 = _chrome$1 ? +_chrome$1[1] : 0;
var safari$1 = !ie$1 && !!nav$1 && /Apple Computer/.test(nav$1.vendor);
var ios$1 = safari$1 && (/Mobile\/\w+/.test(agent$1) || !!nav$1 && nav$1.maxTouchPoints > 2);
var mac$2 = ios$1 || (nav$1 ? /Mac/.test(nav$1.platform) : false);
var android$1 = /Android \d/.test(agent$1);
var webkit$1 = !!doc$1 && "webkitFontSmoothing" in doc$1.documentElement.style;
var webkit_version$1 = webkit$1 ? +(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent) || [0, 0])[1] : 0;
...
function nodeRule(regexp, nodeType, options = {}) {
	return new InputRule(regexp, (state, match, start, end) => {
		...
		if (node) {
			...
			(_c = options.beforeDispatch) == null || _c.call(options, {
...
function posToDOMRect(view, from, to) {
	...
	return __spreadProps(__spreadValues({}, data), { toJSON: () => data });
}
function cloneTr(tr) {
...
var Ctx = class Ctx {
	constructor(container, clock, meta) {
		this.produce = (meta2) => {
			if (meta2 && Object.keys(meta2).length) return new Ctx(this.#container, this.#clock, { ...meta2 });
			return this;
		};
...
var _ParserState = class _ParserState extends Stack {
	constructor(schema) {
		super();
...
var _SerializerStackElement = class _SerializerStackElement extends StackElement {
...
var _SerializerState = class _SerializerState extends Stack {
	constructor(schema) {
		super();
...
function parseFromClipboard(view, text, html, plainText, $context) {
	...
	if (asText) {
		...
		if (inCode) {
			slice = new Slice$1(Fragment.from(view.state.schema.text(text.replace(/\r\n?/g, "\n"))), 0, 0);
...
function closeSlice(slice, openStart, openEnd) {
	if (openStart < slice.openStart) slice = new Slice$1(closeRange(slice.content, -1, openStart, slice.openStart, 0, slice.openEnd), openStart, slice.openEnd);
	if (openEnd < slice.openEnd) slice = new Slice$1(closeRange(slice.content, 1, openEnd, slice.openEnd, 0, 0), slice.openStart, openEnd);
	return slice;
}
var wrapMap = {
	thead: ["table"],
...
function doPaste(view, text, html, preferPlain, event) {
	...
	if (view.someProp("handlePaste", (f) => f(view, event, slice || Slice$1.empty))) return true;
...
//#endregion
//#region node_modules/@milkdown/utils/node_modules/nanoid/index.js
...
function insert(markdown, inline = false) {
	return (ctx) => {
		...
		if (!inline) {
			...
			return view.dispatch(view.state.tr.replaceSelection(new Slice$1(doc.content, contentSlice.openStart, contentSlice.openEnd)).scrollIntoView());
		}
		const schema = ctx.get(schemaCtx);
...
function replaceAll(markdown, flush = false) {
	return (ctx) => {
		...
		if (!flush) {
			...
			return view.dispatch(state2.tr.replace(0, state2.doc.content.size, new Slice$1(doc.content, 0, 0)));
		}
		const schema = ctx.get(schemaCtx);
...
//#endregion
export { AddMarkStep as $, $command, $commandAsync, $ctx, $inputRule, $inputRuleAsync, $mark, $markAsync, $markAttr, $markSchema, $node, $nodeAsync, $nodeAttr, $nodeSchema, $pasteRule, $pasteRuleAsync, $prose, $proseAsync, $remark, $shortcut, $shortcutAsync, $useKeymap, $view, $viewAsync, findNodeInSelection as A, posToDOMRect as B, joinBackward as C, createSlice as D, wrapIn as E, findSelectedNodeOfType as F, AllSelection as G, InputRule as H, getNodeFromSchema as I, Plugin as J, EditorState as K, isTextOnlySlice as L, findParentNode as M, findParentNodeClosestToPos as N, browser as O, findParentNodeType as P, TextSelection as Q, markRule as R, exitCode as S, toggleMark as T, textblockTypeInputRule as U, expectDomTypeError as V, wrappingInputRule as W, Selection as X, PluginKey as Y, SelectionRange as Z, Decoration as _, SerializerReady as a, addTimer, canSplit as at, keydownHandler as b, editorCtx as c, callCommand, DOMParser as ct, parserCtx as d, Mark as dt, Mapping as et, prosePluginsCtx as f, forceUpdate, Node as ft, serializerCtx as g, getHTML, getMarkdown, schemaCtx as h, Slice$1 as ht, InitReady as i, insert, insertPos, canJoin as it, findParent as j, cloneTr as k, editorViewCtx as l, DOMSerializer as lt, rootCtx as m, markdownToSlice, Schema as mt, EditorStatus as n, nanoid, ReplaceStep as nt, commandsCtx as o, findWrapping as ot, outline, remarkStringifyOptionsCtx as p, pipe, NodeRange as pt, NodeSelection as q, EditorViewReady as r, replaceAll, replaceRange, Transform as rt, defaultValueCtx as s, setAttr, liftTarget as st, Editor as t, ReplaceAroundStep as tt, editorViewOptionsCtx as u, Fragment as ut, DecorationSet as v, setBlockType as w, keymap$1 as x, EditorView as y, nodeRule as z };
