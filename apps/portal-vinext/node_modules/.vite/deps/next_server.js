import { r as encodeMiddlewareRequestHeaders, t as parseCookieHeader } from "./parse-cookie-header-yPbBby3v.js";
...
var RequestCookies = class {
	...
	getAll(nameOrOptions) {
		...
		return [...this._parse().entries()].filter(([cookieName]) => name === void 0 || cookieName === name).map(([cookieName, value]) => ({
			name: cookieName,
			value
		}));
	}
...
*/
function after(task) {
	(typeof task === "function" ? Promise.resolve().then(task) : task).catch((err) => {
...
async function connection() {
	const { markDynamicUsage, throwIfInsideCacheScope } = await import("./next_headers.js");
...
//# sourceMappingURL=next_server.js.map